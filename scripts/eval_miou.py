...
def parse_args():
    ...
    parser.add_argument('--gt', help='direction of big gt segmentations', default='data/big/test')
    parser.add_argument('--coarse', help='direction of big coarse segmentations', default='data/big/coarse/fcn')
    parser.add_argument('--refine', help='direction of big refined segmentations', default='big_deeplab')
...
if __name__ == '__main__':
    ...
    with tqdm(total=len(all_gts)) as p:
        for gt_file in all_gts:
            ...
            seg = np.array(Image.open(os.path.join(coarse_root, gt_file.replace('_gt', '_seg'))
                                        ).convert('L'))

            mask = np.array(Image.open(os.path.join(refine_root, gt_file.replace('_gt', '_refine'))
                                        ).convert('L'))
